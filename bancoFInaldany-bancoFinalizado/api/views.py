...
class MaintenanceAPI(APIView):
    ...
    def get(self, request, pk='', status=''):
        ...
        if pk == '':
            ...
            maintenanceResult = Maintenance.objects.filter(idMachineFK=pk, statusMaint=status)
            serializer = MaintenanceTable(maintenanceResult, many=True)
            return Response(serializer.data)
...
# --------------------------Maintenance-----------------------
# ------------------------------------------------------------
class PatchMaintenanceAPI(APIView):

    def patch(self, request, pk):
        maintenanceResult = Maintenance.objects.get(id=pk)
        serializer = PatchMaintenanceTable(maintenanceResult, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data)
#------------------------------------------------------------
