...
class Connection:
    ...
    def update_tag(self, id_Card, idColabEdit):
        urlPatch = self.url + '/associates/' + idColabEdit + '/'

        try:
            patch = {
                "id_card" : id_Card
            }
            urlpaatch = requests.patch(urlPatch, patch)
            print(urlpaatch.status_code)
            if urlpaatch.status_code==200:
                print('tag atualizada')

        except:
            print('deu b.o', urlpaatch.status_code)
...
class Standby(QMainWindow, Ui_Standby):  # primeira tela que faz a validação da tag do crachá
    ...
    def configurar(self):
        print('self.colaboradorB[0]', type(self.colaboradorB[0]))
...
class Aviso_liberacao(QMainWindow, Ui_Aviso_Liberacao):
    ...
    #Função qeu chama a tela de relatar incidente.
    def relatos_liberacao(self):
        relatos_liberacao.textEdit.setFocus()
        liberacao_seguranca.string_da_lista = " ".join(liberacao_seguranca.inconformidades)
...
class Relatos_liberacao(QMainWindow, Ui_Relatos_liberacao):
    ...
    def salvar(self):
        ...
        try:
            url = banco_dados.url + 'observations/'
            ...
            if(urlRelato.status_code == 200):
                ...
                print('deu b.o', urlRelato.status_code)
        except:
            print(urlRelato.status_code)
            print('deu b.o')
...
class Documentos_documentos_de_pecas_menu(QMainWindow, Ui_documentos_documentos_de_pecas_menu):
    ...
    def perfil_de_aluminio_30(self):
...
class Documentos_de_peca_perfil_de_aluminio(QMainWindow, Ui_peca01):
    def __init__(self):
        super().__init__()
        super().setupUi(self)
        self.troca_pagina = 0
        #self.label_titulo.resize(500,60)
        #self.label_imagem.move(100,100)
...
class Cadastros_menu(QMainWindow, Ui_cadastros_menu):
    ...
    def editar(self):
        ...
        try:
            ...
            self.Data_Nasciento = banco_dados.dataNascFormat2
            self.skill = banco_dados.skill
            self.skill2 = banco_dados.skill2
            self.IDedit = banco_dados.idColadorador
            idGlobalEdit = self.IDedit
            print('idGlobalEdit', idGlobalEdit)
...
class Cadastros_menu_editar(QMainWindow, Ui_Editar_cadastro):
    ...
    def atualizar(self):
        ...
        dataNascFormat = datetime.strptime(self.data_nascimento, '%d/%m/%Y').date()
        print('2:', type(dataNascFormat))
        dataNascFormat = datetime.strftime(dataNascFormat, '%Y-%m-%d')

        print(type(dataNascFormat))
        self.jobPosition = cadastros_menu_editar.comboBox.currentText()
        print('-------------- JOB POSITION ------------------', self.jobPosition)
        ...
        if cadastros_menu_editar.cbSkill.isChecked():
            self.skill = True

        else:
            self.skill2 = False

        if cadastros_menu_editar.cbSkill_2.isChecked():
            self.skill2 = True

        else:
            self.skill2 = False

        url = banco_dados.url + 'associates/' + cadastros_menu.IDedit + '/'
        # print('-------------- JOB POSITION ------------------', self.jobPosition)
        ...
        try:
            ...
            print("falha em att:", urlpost.status_code)
        #self.data = datetime.strptime(f'{self.data_nascimento}', '%d/%m/%Y').date()
...
class Cadastros_menu_leitor(QMainWindow, Ui_Aproxime_cartao):
    def __init__(self):
        ...
        self.tag_cartao = " "
        print('------------------------1idGlobalEdit', idGlobalEdit)
    def voltar(self):
    ...
    def registrar(self):
        ...
        if not self.tag_cartao == "":
            if self.adicionar == True:
                ...
                self.lineEdit_tag_cartao.setFocus()
                banco_dados.update_tag(self.tag_cartao, idGlobalEdit)
